 ...
 * <p>
 * This class provides methods for searching customers with pagination and
 * deleting customers by their IDs. It acts as an intermediary between the
 * Action layer and the DAO layer.
 * </p>
 */
public class T002Service {
	...
	/**
	 * Searches for customers based on search conditions (SCO) and handles
	 * pagination logic.
	 * <p>
	 * The method updates the search condition object ({@link T002SCO}) stored in
	 * session, fetches results from the DAO, calculates pagination data, and sets
	 * both results and pagination attributes to the request for rendering in the
	 * view.
	 * </p>
	 *
	 * @param form    The {@link T002Form} containing input search criteria and
	 *                pagination info.
	 * @param request The {@link HttpServletRequest} used to retrieve parameters and
	 *                set attributes.
	 * @param session The {@link HttpSession} used to store and retrieve the search
	 *                condition object.
	...
	public void searchCustomers(T002Form form, HttpServletRequest request, HttpSession session) throws SQLException {
		...
		Object customersObj = data.get(Constants.PRAM_CUSTOMERS);
		List<T002Dto> customers;

		if (customersObj instanceof List) {
			customers = ((List<?>) customersObj).stream().filter(T002Dto.class::isInstance).map(T002Dto.class::cast)
					.collect(Collectors.toList());
		} else {
			customers = Collections.emptyList();
		}

		int totalRecords = (int) data.get(Constants.PARAM_TOTAL_COUNT);
	 ...
	 * <p>
	 * This method receives an array of customer IDs, converts it to a list, and
	 * delegates the deletion to the DAO layer.
	 * </p>
	 *
